...
public class StatusBarManager {
    ...
    public static void translucentStatusBar(Activity activity) {
        ...
        if (SystemFacade.hasKitKat() && !SystemFacade.hasLollipop()) {
            ...
            /*
                FULLSCREEN  全屏显示
                STABLE  稳定
                View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN 系统用户界面标志布局全屏显示
                View.SYSTEM_UI_FLAG_LAYOUT_STABLE   系统界面标志布局稳定
             */
            //所以布局延伸到了状态下面
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);//标志_半透明_状态

        } else if (SystemFacade.hasLollipop()) {
            Window window = activity.getWindow();
            //添加Flag把状态栏设为可绘制模式
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);//标志_绘图_系统_栏_背景
    ...
    public static void immersive(Activity activity) {
        ...
        if (SystemFacade.hasKitKat() && !SystemFacade.hasLollipop()) {
            ...
            window.getDecorView().setSystemUiVisibility(
                    View.SYSTEM_UI_FLAG_LAYOUT_STABLE//系统界面标志布局稳定
                            | View.SYSTEM_UI_FLAG_FULLSCREEN//系统用户界面标志全屏显示
                            | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION//系统用户界面标志隐藏导航
                            | View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY//系统_用户界面_标志_沉浸式_粘性
                            | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);//系统用户界面标志布局全屏显示
    ...
    private static void createOrChangeMockStatusBarColor(Activity activity, int color) {
        /*
            4.4创建一个假的Window，设置颜色，设置到状态栏的位置
         */
        Window window = activity.getWindow();
